(function () {
  ...
  // Applies the project's theme to CSS variables so the gift renders in the
  // colors, background, and font chosen in the editor. Without a theme the
  // stylesheet defaults remain in effect.
  function applyTheme(theme) {
    ...
    if (theme.backgroundImage) {
      ...
      root.setProperty('--theme-bg', theme.backgroundCss);
    }
    if (theme.palette) {
  ...
  // /g/{slug}; ?g={slug} is also accepted as a convenience for local testing.
  function resolveSlug() {
    var url = new URL(window.location.href);
    var fromQuery = url.searchParams.get('g');
    if (fromQuery) return fromQuery;
    var match = url.pathname.match(/\/g\/([^/]+)/);
    return match ? decodeURIComponent(match[1]) : null;
  }

  // Loads the published gift's config + theme from Firestore by slug. The slug
  // maps to a projectId via the public `slugs` collection; the project is only
  // readable here once it has been published (enforced by Firestore rules).
  function loadConfig() {
    var slug = resolveSlug();
    if (!slug) return Promise.reject(new Error('Kein Geschenk angegeben.'));
    state.slug = slug;

    if (!window.firebase || !window.firebase.firestore) {
      return Promise.reject(new Error('Firestore SDK fehlt.'));
    }
    ...
    return fs.collection('slugs').doc(slug).get()
      .then(function (slugSnap) {
        if (!slugSnap.exists) throw new Error('Geschenk nicht gefunden.');
        var projectId = slugSnap.data().projectId;
        return fs.collection('projects').doc(projectId).get();
      })
      .then(function (projSnap) {
        if (!projSnap.exists) throw new Error('Geschenk nicht gefunden.');
        var data = projSnap.data();
        if (!data.published) throw new Error('Dieses Geschenk ist noch nicht veröffentlicht.');
  ...
  function showLoadError() {
    app.innerHTML =
      '<div class="card slide-in">' +
        '<div class="question">Hoppla — dieses Geschenk konnte nicht geladen werden.</div>' +
        '<div class="hint">Pruefe den Link. Vielleicht wurde das Geschenk noch nicht veroeffentlicht.</div>' +
      '</div>';
  }
  ...
  function activateRole(role) {
    ...
    if (window.Realtime) {
      window.Realtime.init(state.slug).then(function () {
